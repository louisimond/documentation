---
title: "Overview"
description: "Connect your existing phone system and numbers to Kalla AI"
---
...
SIP (Session Initiation Protocol) integration allows you to connect your existing phone system to Kalla AI. Think of it as building a bridge between your current phone setup and our AI platform. This means you can:
...
When you integrate your phone system with Kalla AI:
...
You can connect your phone system to Kalla AI in two ways:
...
1. The credentials you set in Kalla AI dashboard must match exactly with your VoIP configuration
...
<Warning>
  ...
  Kalla AI dashboard (and vice versa) to maintain the connection.
</Warning>
...
1. Go to your Kalla AI dashboard
...
These settings control how Kalla AI sends outbound calls through your SIP provider.
...
- **Outbound Proxy (Fixed IP)** - Some SIP providers require calls to originate from a single, whitelisted IP address. Configure this as follows:
  - **Toggle ON** - If your SIP provider requires a fixed source IP, enable Outbound proxy will come from a fixed IP address. Kalla AI will then send all outbound requests through a known static IP.
  Use this option only if your provider explicitly requires a single source IP.
  - **Toggle OFF:** - If your provider accepts calls from any IP, leave this option off. In that case, Kalla AI will send calls from dynamic IPs.
...
These settings determine how Kalla AI receives and authenticates inbound calls from your SIP provider.

1. **Our SIP Address** - Kalla AI assigns a unique inbound SIP URI ```5wed79aeavr.sip.livekit.cloud```. Your SIP provider must be configured to send inbound calls to this address by pointing their outbound trunk or routing rule to this URI.
